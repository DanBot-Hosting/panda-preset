...
import type { Option, Recursive } from "@/types/utils";
import { themeModes } from "@/lib/themeModes";
import { Filter, Palette, stringify } from "cvet";
import type { Amount, HEX } from "cvet/types";
import type {
  GenerateThemeColorsAccumulator,
  ModifiedColor,
  Theme,
  ThemeModes,
...
export function extendModifiedColor(
  ...
  return tokens.reduce<Record<number, { value: UnwrappedModifiedColor }>>(
    (acc, token) => {
      return {
        ...acc,
        [token]: {
...
/**
 * Generates an array of Option objects based on the themes array.
 *
 * @return {Option[]} An array of Option objects.
 */
export function generateThemeOptions(): Option[] {
  ...
  for (let i = 1; i < 5; i++) {
    ...
    for (let theme in themes) {
      part.value[`_${themes[theme].value}`] =
        themeModes.reduce<GenerateThemeColorsAccumulator>(
          (acc, mode) => ({
            ...acc,
